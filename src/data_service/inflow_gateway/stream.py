"""Streaming gateway for Polymarket market and reference price feeds."""
...
class MarketDescriptor:
    """Market subscription details derived from the Gamma event catalog."""
...
class RedisPublisher:
    ...
    async def publish(self, channel: str, payload: object) -> None:
        """Publish a JSON-serializable payload to a Redis pub/sub channel."""
...
def _extract_timestamp(slug: str) -> Optional[int]:
    """Return the trailing UNIX timestamp encoded in a market slug, if present."""
...
def _normalise_token_ids(value: object) -> Optional[List[str]]:
    """Normalize token IDs that may arrive as a list or a JSON-encoded list."""
...
def fetch_events(tag_id: Optional[str] = None, closed: str = "false") -> List[dict]:
    """Fetch all Gamma events for the given filters by following pagination."""
    ...
    events: List[dict] = []
    while True:
        # Gamma uses offset pagination, so keep advancing until an empty page is returned.
...
def collect_markets_for_asset(
    ...
    """Collect and sort market descriptors whose slug matches the asset keyword."""
    ...
    for event in events:
        ...
        for market in event.get("markets", []):
            # Some market entries omit their own slug, so fall back to the parent event slug.
...
def _ensure_meta(payload: object) -> Optional[dict]:
    """Return a mutable metadata mapping for dict payloads, creating one if needed."""
...
def _classify_market_window(market: MarketDescriptor) -> str:
    """Classify a market as future, active, or expired using configured timing rules."""
...
def _seconds_until_market_start(market: MarketDescriptor) -> float:
    """Return the non-negative delay before the market stream should begin."""
...
async def populate_market_queue(
    ...
    """Keep a queue stocked with upcoming markets for a single asset."""

    seen_slugs: Set[str] = set()

    while not stop_event.is_set():
        # Avoid refetching while enough queued markets are already buffered.
        ...
        # Deduplicate across refresh cycles so each market is streamed at most once.
...
async def stream_single_market(
    ...
    """Stream one market websocket and publish enriched payloads to Redis."""
        ...
        try:
            async with websockets.connect(MARKET_WS_URL, ping_interval=None) as ws:
                ...
                while not stop_event.is_set():
                    ...
                    if meta is None:
                        # Downstream consumers expect metadata even for non-dict payloads.
                        payload_to_publish = {
                            "raw_payload": payload,
                            "meta": {
...
async def stream_markets_for_asset(
    ...
    """Consume queued markets for one asset and run each market stream concurrently."""
    ...
    async def _run_market(market: MarketDescriptor) -> None:
        try:
            ...
            # Always mark the queue item complete, even if startup or streaming fails.
            queue.task_done()
...
async def stream_live_feed(
    ...
    """Stream a shared live-data feed and publish enriched payloads to Redis."""
        ...
        try:
            async with websockets.connect(LIVE_DATA_WS_URL, ping_interval=None) as ws:
                ...
                while not stop_event.is_set():
                    ...
                    meta = _ensure_meta(payload)
                    if meta is None:
                        # Keep a consistent envelope when upstream sends a non-object payload.
                        payload_to_publish = {
                            "raw_payload": payload,
...
async def run_gateway() -> None:
    """Start all live-feed and market-stream tasks until shutdown is requested."""
...
def main() -> None:
    """Run the streaming gateway using the default asyncio event loop."""
