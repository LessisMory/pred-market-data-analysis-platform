        ...
        if isinstance(price_changes, list) and price_changes:
            for change in price_changes:
                ...
                if token_id and price is not None and size is not None and update_ts is not None:
                    dedupe_key = _dedupe_key(token_id, price, size, side_value, update_ts)
                    records.append(
                        (
...
def _dedupe_key(token_id: str, price: float, size: float, side: str, update_ts: datetime) -> str:
    ...
    canonical = "|".join(
        [
            ...
            repr(size),
            side,
            update_ts.isoformat(timespec="milliseconds"),
        ]
    )
