...
const MOCK_USERS = [
  { id: "U-001", name: "Jane Doe",      email: "jane@example.com",      plan: "Pro",   joined: "Nov 28, 2025", lastLogin: "Apr 03, 2026", trades: 87,  status: "ACTIVE"   },
  { id: "U-002", name: "Carlos Mendes", email: "carlos@tradelab.io",    plan: "Elite", joined: "Oct 12, 2025", lastLogin: "Apr 03, 2026", trades: 214, status: "ACTIVE"   },
  { id: "U-003", name: "Priya Nair",    email: "priya@hedgefund.ai",    plan: "Elite", joined: "Sep 05, 2025", lastLogin: "Apr 01, 2026", trades: 332, status: "ACTIVE"   },
  { id: "U-004", name: "Tom Brauer",    email: "tom@btcalgo.com",       plan: "Pro",   joined: "Dec 14, 2025", lastLogin: "Mar 29, 2026", trades: 45,  status: "ACTIVE"   },
  { id: "U-005", name: "Lena Fischer",  email: "lena@quant.de",         plan: "Free",  joined: "Jan 02, 2026", lastLogin: "Mar 20, 2026", trades: 3,   status: "DISABLED" },
  { id: "U-006", name: "Marco Rossi",   email: "marco@pmfund.it",       plan: "Pro",   joined: "Feb 18, 2026", lastLogin: "Apr 02, 2026", trades: 29,  status: "ACTIVE"   },
  { id: "U-007", name: "Aiko Tanaka",   email: "aiko@algo.jp",          plan: "Elite", joined: "Aug 30, 2025", lastLogin: "Apr 03, 2026", trades: 501, status: "ACTIVE"   },
  { id: "U-008", name: "Dave Kim",      email: "dave@polybot.us",       plan: "Free",  joined: "Mar 10, 2026", lastLogin: "Mar 15, 2026", trades: 0,   status: "DISABLED" },
];
...
const AllUsersTab = ({ users, searchQuery, setSearchQuery }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "80px 1fr 1.5fr 80px 1fr 1fr 80px 100px", padding: "12px 24px", background: `${C.bg}80`, borderBottom: `1px solid ${C.border}` }}>
          {["ID","NAME","EMAIL","PLAN","JOINED","LAST LOGIN","TRADES","STATUS"].map(h => <ColHeader key={h}>{h}</ColHeader>)}
        </div>

        {filtered.map((u, i) => (
          <div
            key={u.id}
            style={{ display: "grid", gridTemplateColumns: "80px 1fr 1.5fr 80px 1fr 1fr 80px 100px", padding: "18px 24px", borderBottom: i < filtered.length - 1 ? `1px solid ${C.border}50` : "none", alignItems: "center", transition: "background 0.15s", cursor: "default" }}
...
const PlatformAnalysisTab = ({ data }) => {
  const maxTrades = Math.max(...data.topTraders.map(t => t.trades));
  const maxDau    = Math.max(...data.dau.map(d => d.count));
...
const AdminScreen = () => {
  ...
  const handleToggleStatus = async (userId) => {
    ...
    //   await fetch(`https://api.yourbackend.com/api/analytics/admin/users/${userId}/status`, {
    //     method: 'PATCH',
  ...
  const handleResetPassword = async (user) => {
    ...
    alert(`[Demo] Password reset link sent to ${user.email}`);
  };
    ...
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: C.bg, color: C.white, fontFamily: "'DM Sans'" }}>
      ...
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 32px", background: C.surface, borderBottom: `1px solid ${C.border}` }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <button
            onClick={() => { localStorage.removeItem('ob_user_name'); localStorage.removeItem('jwt_token'); localStorage.removeItem('ob_user_role'); window.location.href = 'index.html'; }}
      ...
      <main style={{ flex: 1, padding: "32px", maxWidth: 1400, margin: "0 auto", width: "100%" }}>
        {isLoading ? (
          ...
          <>
            {activeTab === 'users'        && <AllUsersTab users={users} searchQuery={searchQuery} setSearchQuery={setSearchQuery} />}
            ...
            {activeTab === 'platform'     && <PlatformAnalysisTab data={MOCK_PLATFORM} />}
          </>
        )}
      </main>
    </div>
